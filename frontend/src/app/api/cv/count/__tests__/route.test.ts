...
// (replaces the previous Python backend proxy). We mock
// `@/lib/supabase/server` to return a chainable query-builder stub
// whose `.then`-resolution shape mirrors PostgREST: `{ count, error }`.
// The mock pattern is the same as `supabase-queries.test.ts`.
...
describe("GET /api/cv/count", () => {
  it("returns total_today=0 when the user is not authenticated", async () => {
    ...
    const body = (await res.json()) as { total_today: number };
    expect(body.total_today).toBe(0);
    // No Supabase query when there's no session — privacy + cost.
    expect(mockSupabase.from).not.toHaveBeenCalled();
  });

  it("queries user_engagement filtered by event_type and today UTC", async () => {
    mockAuth.getSession.mockResolvedValueOnce({
      data: {
    ...
    const body = (await res.json()) as { total_today: number };
    expect(body.total_today).toBe(3);
  });

  it("returns total_today=0 on Supabase error (graceful degradation)", async () => {
    mockAuth.getSession.mockResolvedValueOnce({
      data: {
        session: { user: { id: "user-1" } },
      },
  ...
  it("returns total_today=0 when count is null but no error", async () => {
    mockAuth.getSession.mockResolvedValueOnce({
      data: {
        session: { user: { id: "user-1" } },
      },
