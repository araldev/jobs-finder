...
"LINKS — URL RULES: include the URL VERBATIM from the original CV. Do NOT invent URLs (the no-hallucination rule still applies). If the original CV has a labeled link (e.g. 'V12-UI | Github link | Storybook link') but the PDF has NO actual hyperlink annotation for one of those labels (the label is just plain text, not a clickable link), STILL emit that link entry with `label` set and `url` set to empty string `\"\"` — the renderer shows a label-only chip so the user SEES the link exists in the original CV, even without a clickable URL target. NEVER invent a URL based on a label. The parser caps `links` at 8 entries per project (more entries than 8 are dropped silently).\n" +
  ...
  "4. PROJECT DESCRIPTION KEYWORD DENSITY (for ATS, not for hallucination): each bullet of a project's description should mention at least ONE technology from the project's `technologies` array (rephrased, not invented — only use techs that ARE in the technologies array). This gives ATS more keyword density without inventing skills. Example: a project with `technologies: [\"React\", \"TypeScript\"]` should have description bullets like \"• Built React components with TypeScript interfaces…\" — the keyword mention comes from the technologies array, not from the job description.\n" +
  "\n" +
