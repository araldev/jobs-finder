...
"LINKS — URL RULES: include the URL VERBATIM from the original CV. Do NOT invent URLs (the no-hallucination rule still applies). If the original CV has a labeled link (e.g. 'V12-UI | Github link | Storybook link') but the PDF has NO actual hyperlink annotation for one of those labels (the label is just plain text, not a clickable link), STILL emit that link entry with `label` set and `url` set to empty string `\"\"` — the renderer shows a label-only chip so the user SEES the link exists in the original CV, even without a clickable URL target. NEVER invent a URL based on a label. The parser caps `links` at 8 entries per project (more entries than 8 are dropped silently).\n" +
  "LINKS — USE ORIGINAL URLs VERBATIM (CRITICAL):\n" +
  "When the user message contains a HYPERLINKS — ORIGINAL URL MAP section, you MUST use the URLs from that MAP for every project link. The MAP contains the REAL URLs embedded in the original CV's PDF link annotations — those are the URLs the user wants, not URLs you invent from labels. NEVER invent a URL based on a label (e.g. do NOT synthesize `https://github.com/<project>` from a label that says 'GitHub' — that is hallucination, even when the URL looks plausible). If a label in the original CV has no matching hyperlink in the MAP (because the CV's PDF has no hyperlink annotation for that label, only plain text), still emit the link with the label and `url: \"\"` (empty string) — the renderer shows a label-only chip so the user still SEES the link in the adapted CV. Each entry in `links[]` MUST have a non-empty `label`; the `url` MAY be `\"\"` when the original CV has no URL target for that label.\n" +
  ...
  "2. Combine multiple roles at the same company (if the original CV shows multiple roles at the same company, combine them into ONE entry with ONE description).\n" +
  "3. NO KEYWORD INJECTION — THE HARDEST RULE (CRITICAL, ABSOLUTE):\n" +
  "  Do NOT add skills, technologies, or keywords to the skills array (or anywhere else) that are NOT EXPLICITLY mentioned verbatim in the original CV. The job description is NOT a valid source of new skills. If the original CV does not mention MySQL, do NOT add MySQL to the skills array even if the job requires MySQL. If the original CV mentions PostgreSQL, do NOT swap it for MySQL. If the original CV does not mention any cloud platform (AWS / Azure / GCP), do NOT add one. The skills array MUST contain ONLY items that appear VERBATIM in the original CV (either in the skills section, projects, experience descriptions, certifications, education, etc.). The LLM is NOT authorized to infer skills from context — only copy them verbatim. Example of VERBATIM copy: CV says \"managed MySQL databases\" → emit \"MySQL\". Example of FORBIDDEN inference: CV says \"managed databases\" + JD requires MySQL → do NOT add MySQL. Example of FORBIDDEN swap: CV has PostgreSQL + JD requires MySQL → keep PostgreSQL, do NOT swap to MySQL. This rule is more important than ATS keyword matching or any other consideration — telling the user they have a skill they don't have is a HALLUCINATION and a misleading CV.\n" +
  "4. PROJECT DESCRIPTION — preserve original details FIRST, then add keyword density (for ATS, not for hallucination):\n" +
  "  PRIORITY: the description MUST preserve the original CV's project details (what the project does, who it serves, what concrete features it has, the role the candidate played). DO NOT substitute the original details with generic tech-mention sentences — the user explicitly complained that this caused information loss.\n" +
  "  SECONDARY: if the description bullets are already rich with the original details, the ATS keyword density is sufficient from the dedicated `Tecnologías:` line and the per-link chip row — you do NOT need to add a tech mention to every bullet. If the original details don't naturally include a technology, MENTION IT IN ONE BULLET ONLY (a single mention is enough for ATS; multiple substitutions erase the user's project story).\n" +
  "  Example GOOD: a V12-UI project with `technologies: [\"React\", \"TypeScript\"]` has original bullets like '• Componente Button con 3 variantes y validación de props' — the renderer shows React / React.js / ReactJS as a chip below the name, so the bullet keeps the original detail. ONE of the 3-4 bullets can mention '• Documentación con TypeScript estricto' (the tech mention is natural, not a substitution).\n" +
  "  Example BAD: replacing the original bullets with '• Built React components with TypeScript interfaces' — the user loses the original detail (the Button component with 3 variants) and the project description becomes a hollow tech showcase. DO NOT do this.\n" +
  "\n" +
  ...
  "\n" +
  "SUMMARY RULE (REQUIRED — ATS OPTIMIZATION):\n" +
  "The output MUST include a non-empty \"summary\" string of 2-3 sentences. The FIRST sentence MUST start with 3-5 KEY TECHNICAL KEYWORDS (extracted VERBATIM from the original CV's skills/projects/experience text — NEVER from the job description), separated by commas or joined with \"y\". Example: \"Desarrollador web con experiencia en React, TypeScript y Node.js. Aplica esos stack a problemas reales — diseñado para entregar valor medible. En los últimos meses llevó 2 productos a producción.\" The FIRST sentence is the ATS-read-first line (ATS rankers weight top-of-CV content heavily). Two cases for the rest of the summary:\n" +
  "  (a) If the original CV has a summary paragraph anywhere in the document (a \"Perfil\" / \"Summary\" / \"Professional Profile\" / \"Acerca de\" / \"Profile\" section, or a few lines of self-description at the top or bottom of the CV), extract 1-2 sentences of that paragraph verbatim and put them as sentences 2-3 in the summary. Rephrase action verbs to be stronger if needed, but do NOT change facts.\n" +
  "  (b) If the original CV has no summary at all, build sentences 2-3 by REPHRASING content that IS in the original CV (e.g. the most recent job title + years of experience + the primary field). Do NOT invent: every fact in the summary must be derivable from the original CV.\n" +
  "  HARD: keywords in the first sentence MUST come VERBATIM from the original CV text. Do NOT add MongoDB to the first sentence if the CV only mentions PostgreSQL. Use what's there. The first sentence puts these keywords in the ATS-read-first position so the CV ranks for them.\n" +
  "  The output's \"summary\" field MUST be a non-empty string.\n" +
  "\n" +
  ...
  "- skills array: ONLY items that appear VERBATIM in the original CV. NO additions from the job description (see NO KEYWORD INJECTION rule above — this is the legal/version-safe baseline; telling the user they have a skill they don't have is a HALLUCINATION that breaks the CV).\n" +
...
export interface AdaptedCV {
   ...
   *
   * Each entry has a `name` (the verbatim text from the original
   * CV) and an optional `url` (the link to the certificate /
   * course, if the original CV has one). `url` is `null` for certs
   * without a hyperlink in the original CV (e.g. a "Carné de
   * conducir B" with no link). The renderer shows a small "↗"
   * indicator next to certs that have a URL and makes the cert
   * name clickable.
   *
   * Mirrors `CertificationEntry` in
   * `backend/src/jobs_finder/infrastructure/cv/_template.py`.
   */
  certifications: AdaptedCVCertification[];
  skills: string[];
  languages: string[];
  /**
 ...
 */
export interface AdaptedCVCertification {
  name: string;
  url: string | null;
}
