...
# --- Camino 1: _apply_snippet_map helper (REQ-SCRAPER-INDEED-SNIPPET-001) ---


def test_apply_snippet_map_populates_description_from_json() -> None:
    """Each job's description is replaced with the JSON snippet when the jobkey matches.

    The helper reads the JSON-embedded `providerData` snippets
    and applies them to the parsed jobs. A matching jobkey
    triggers a description replacement; a missing jobkey
    keeps the job's `description=None` (v1 contract).
    """
    from datetime import UTC, datetime  # noqa: PLC0415

    from jobs_finder.domain.job import Job  # noqa: PLC0415
    from jobs_finder.infrastructure.indeed.scraper import (  # noqa: PLC0415
        _apply_snippet_map,
    )
    ...
    jobs = [
        Job(
            id=SNIPPET_JOBKEYS[0],  # matches a snippet
            title="X", company="X", location="X",
            url="https://es.indeed.com/viewjob?jk=" + SNIPPET_JOBKEYS[0],
            posted_at=datetime(2026, 1, 1, tzinfo=UTC),
            description=None,
        ),
        Job(
            id="not-in-snippet-map",  # does not match
            title="Y", company="Y", location="Y",
            url="https://es.indeed.com/viewjob?jk=not-in-snippet-map",
            posted_at=datetime(2026, 1, 1, tzinfo=UTC),
            description=None,
        ),
    ]
    enriched = _apply_snippet_map(PAGE_HTML, jobs)
    assert len(enriched) == 2
    # First job: description populated from the snippet
    assert enriched[0].description is not None
    assert "Especialista en Inteligencia Artificial" in enriched[0].description
    # Second job: no match, description stays None
    assert enriched[1].description is None
    # Immutability: original jobs are NOT mutated
    assert jobs[0].description is None
    assert jobs[1].description is None


def test_apply_snippet_map_empty_page_html_returns_jobs_unchanged() -> None:
    """Empty page HTML → jobs are returned unchanged (no raise)."""
    from datetime import UTC, datetime  # noqa: PLC0415
    ...
    jobs = [
        Job(
            id="abc", title="X", company="X", location="X",
            url="https://es.indeed.com/viewjob?jk=abc",
            posted_at=datetime(2026, 1, 1, tzinfo=UTC),
            description=None,
        ),
    ]
    result = _apply_snippet_map("", jobs)
    assert len(result) == 1
    assert result[0].description is None


def test_apply_snippet_map_no_anchor_returns_jobs_unchanged() -> None:
    """A page without the providerData anchor → jobs returned with description=None."""
    from datetime import UTC, datetime  # noqa: PLC0415
    ...
    page_html = "<html><head><script>console.log('no mosaic');</script></head></html>"
    result = _apply_snippet_map(page_html, jobs)
    assert result[0].description is None
