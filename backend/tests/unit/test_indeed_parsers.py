...
# --- Camino 1: snippet extraction from providerData JSON (REQ-PARSER-INDEED-SNIPPET-001) ---


def test_extract_indeed_snippets_from_provider_data_real_fixture() -> None:
    """The captured real providerData JSON is parsed into {jobkey: snippet} map.

    Sanity check: the fixture
    (`tests/fixtures/indeed_provider_data_jobcards.py`) contains
    3 real Indeed job entries with `jobkey` and `snippet`
    fields. The parser must:
    - return a dict with 3 entries,
    - each entry has a non-empty text (>= 100 chars, the
      shortest snippet observed in production),
    - the text contains Spanish/English words from the
      original job post.
    """
    from tests.fixtures.indeed_provider_data_jobcards import (  # noqa: PLC0415
        PAGE_HTML,
        SNIPPET_JOBKEYS,
    )

    result = extract_indeed_snippets_from_provider_data(PAGE_HTML)
    assert len(result) == len(SNIPPET_JOBKEYS)
    for jk in SNIPPET_JOBKEYS:
        text = result.get(jk)
        assert text is not None, f"missing snippet for jobkey {jk}"
        assert len(text) >= 100, f"snippet too short ({len(text)} chars) for {jk}"
    # The first snippet in the fixture is the "Especialista en IA con Python" job.
    first_text = result[SNIPPET_JOBKEYS[0]]
    assert "Especialista en Inteligencia Artificial" in first_text
    assert "Python" in first_text


def test_extract_indeed_snippets_from_provider_data_empty_page() -> None:
    """Empty page HTML returns an empty dict (no raise)."""
    assert extract_indeed_snippets_from_provider_data("") == {}
    assert extract_indeed_snippets_from_provider_data(None) == {}


def test_extract_indeed_snippets_from_provider_data_anchor_missing() -> None:
    """A page without the providerData anchor returns an empty dict."""
    page_html = "<html><head><script>console.log('no mosaic here');</script></head></html>"
    assert extract_indeed_snippets_from_provider_data(page_html) == {}


def test_extract_indeed_snippets_from_provider_data_malformed_json() -> None:
    """A page where the anchor is followed by malformed JSON returns an empty dict."""
    page_html = (
        '<html><head><script>'
        'window.mosaic.providerData["mosaic-provider-jobcards"]={not valid json at all};'
        '</script></head></html>'
    )
    assert extract_indeed_snippets_from_provider_data(page_html) == {}


def test_extract_indeed_snippets_from_provider_data_empty_joblistings() -> None:
    """A page where the JSON is valid but contains no jobkey entries returns empty dict."""
    page_html = (
        '<html><head><script>'
        'window.mosaic.providerData["mosaic-provider-jobcards"]={"metaData": {"isJpBundle": false}};'
        '</script></head></html>'
    )
    assert extract_indeed_snippets_from_provider_data(page_html) == {}


def test_extract_indeed_snippets_from_provider_data_multiple_li() -> None:
    """A snippet with multiple `<li>` items is joined with " | ".

    Indeed renders some snippets as multi-bullet lists
    (observed in 1/15 of the live capture: 1-2 items per
    snippet, typically 1). The parser joins them with " | "
    so the LLM downstream gets a single scrubbable string
    (same shape as `parse_indeed_description`).
    """
    snippet_html = (
        '<ul style="list-style-type:circle;">'
        '<li style="margin-bottom:0px;">First bullet of the snippet</li>'
        '<li>Second bullet of the snippet</li>'
        '</ul>'
    )
    import json as _json
    fixture = {
        "jobListings": [
            {"jobkey": "test123", "snippet": snippet_html},
        ],
    }
    page_html = (
        '<html><head><script>'
        f'window.mosaic.providerData["mosaic-provider-jobcards"]={_json.dumps(fixture, ensure_ascii=False)};'
        '</script></head></html>'
    )
    result = extract_indeed_snippets_from_provider_data(page_html)
    assert "test123" in result
    text = result["test123"]
    assert "First bullet of the snippet" in text
    assert "Second bullet of the snippet" in text
    assert " | " in text
