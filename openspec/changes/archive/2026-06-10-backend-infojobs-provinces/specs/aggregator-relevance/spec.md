# Delta for `aggregator-relevance` — Keep `filter_infojobs_results` as defense-in-depth

> **Delta type**: MODIFIED. The base spec for `aggregator-relevance`
> does NOT exist in `openspec/specs/`; this delta documents the
> KEEP-and-document decision (the user's Q3 answer) on top of the
> pre-change `filter_infojobs_results` contract (defined in
> `infrastructure/aggregator_filters.py`, added in
> `backend-scraper-query-tuning` PR #4 merged 2026-06-10).
...
None. No existing requirement is removed. The filter function
stays alive (Q3 = KEEP), and no semantic change is made to the
filter logic.
