# Delta for `infojobs-scraper` — Plumb `provinceIds` + `countryIds` into the URL

> **Delta type**: MODIFIED. The base spec for `infojobs-scraper` does
> NOT exist in `openspec/specs/` (the project has only `chat-streaming`
> and `frontend-scaffold`); this delta documents the new behavior on
> top of the pre-change `InfoJobsPlaywrightScraper` contract.
> Archive will migrate the documented behavior into the main spec
> once it is created by a future change.
...
> The pre-change `InfoJobsPlaywrightScraper.search(...)` signature is
...
None. No existing requirement is removed — only new requirements
are added; the v1 URL formula is preserved as the fallback path.
