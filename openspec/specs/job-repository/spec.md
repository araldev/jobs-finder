...
`application/ports.py` with three async methods. Structural subtyping
...
- GIVEN a class `SqliteJobRepository` with `upsert_jobs`, `search_jobs`,
  and `close` methods matching the Protocol signatures
...
| REQ | Scenarios | Count |
|-----|-----------|-------|
| REQ-DB-001 | Protocol conformance, rejection of missing method | 2 |
