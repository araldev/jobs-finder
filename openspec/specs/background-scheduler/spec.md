...
| `scheduler_queries` | `SCHEDULER_QUERIES` / `scheduler_queries` | `list[dict[str, str]]` | `[{"keywords": "", "location": "Madrid"}, {"keywords": "", "location": "Barcelona"}, {"keywords": "", "location": "España"}]` |
...
### REQ-HOURS-001 — Scheduler checks Madrid time before each cycle

(Modified 2026-06-13 — `scheduler-source-fix` delta — previously the scheduler ran continuously without time restrictions.)
...
### REQ-HOURS-002 — Outside 09:00-22:00: sleeps 5 min intervals
...
### REQ-HOURS-003 — At 22:00: stops and waits until 09:00 next day
...
### REQ-HOURS-004 — At 09:00: proceeds immediately with cycle
...
### REQ-QUERY-002 — .env.example documents empty keywords format

(Added 2026-06-13 — `scheduler-source-fix` delta.)
...
### REQ-QUERY-003 — Scrapers handle empty keywords without error

(Added 2026-06-13 — `scheduler-source-fix` delta. Covered by `linkedin-scraper`, `indeed-scraper`, and `infojobs-scraper` delta specs.)
...
| REQ-HOURS-001 | Cycle proceeds during active window, outside window re-checks | 2 |
| REQ-HOURS-002 | Outside window sleeps 5 min and re-checks | 1 |
| REQ-HOURS-003 | At 22:00 sleeps until next day 09:00, edge case end of month | 2 |
| REQ-HOURS-004 | At 09:00 proceeds immediately, edge case first of month | 2 |
| REQ-QUERY-002 | .env.example documents empty keywords format | 1 |
| REQ-QUERY-003 | Uses zoneinfo without additional dependencies | 1 |
| **Total** | | **32** |
