...
The system MUST add 4 new fields to `Settings` in
...
The system MUST wire the scheduler in `app_factory.build_app()` when
`settings.scheduler_enabled` and the aggregator use case are available.
The order:

1. Build `SqliteJobRepository(db_path=settings.db_path)`
2. Open it (`__aenter__`) in the lifespan
3. Build `BackgroundJobScheduler(search_fn=aggregator.search, repo=repo,
   queries=settings.scheduler_queries, min_interval=..., max_interval=...)`
...
- THEN the lifespan creates the repository, opens it, starts the
  scheduler, stops it, and closes the repository
...
| REQ | Scenarios | Count |
|-----|-----------|-------|
| REQ-SCH-001 | Constructor stores params | 1 |
...
| REQ-SCH-005 | Multiple queries, result persists | 2 |
| REQ-CFG-001 (scheduler) | JSON env var parsing | 1 |
| REQ-ROOT-001 | Wired enabled, disabled no-op | 2 |
| **Total** | | **11** |
