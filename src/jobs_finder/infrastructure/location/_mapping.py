"""Canonical geoId mapping (the 34 entries the resolver indexes into).

Sourced from `tests/fixtures/linkedin_geo_ids.csv` rows 2-44
(the 43 captured geoIds from the 2026-06-08 capture). The 9
...
so the resolver returns `None` for them (with a WARNING log):
    ...
    - row 28: `es` → España (worldwide fallback geoId 103644278
      would return globally-distributed results — out of scope)
...
stripped, NFC composed). The 34 keys correspond to:
